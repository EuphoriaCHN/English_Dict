...
import { HTTP_STATUS_CODE, STATUS_CODE } from '@/common/utils/constants';
...
const onAxiosInstanceFulfilled = async (value: AxiosResponse<ResponseData>) => {
  ...
  const { data, errorMessage, statusCode } = value.data;
...
const onAxiosInstanceRejected = async (error: ResponseError): Promise<any> => {
...
export abstract class API {
  ...
  private static _useHooks<Request, Response>(
    ...
    return function (configs = {}, _deps = []) {
      ...
      const [data, setData] = useState<Response | undefined>(init);
      ...
      return { data: data, loading, error, start };
    }
  }
  ...
  public static sign<T = any, R = any>(
    ...
    const request: Signature<T, R> = async function (params) {
      // 参数格式化
      switch ((requestOption.method || 'get').toLowerCase()) {
        case 'post':
